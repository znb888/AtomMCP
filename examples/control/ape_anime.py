# -*- coding: utf-8 -*-
"""
ape_anime.py - 演示多脉冲序列下的量子态演化

本程序通过以下步骤演示了如何使用 quantum_sim 库来仿真一个多脉冲序列，
并观察系统在布洛赫球面上的演化。
"""
...
inverse_pi_half_pulse_env = CosineEnvelope(duration=PI_HALF_DURATION, amp=-PI_HALF_AMP)
...
# # c) 使用 '+' 拼接成基本单元
# base_unit_env = inverse_pi_half_pulse_env + idle_env + pi_half_pulse_env

# # d) 重复基本单元形成完整序列
# full_sequence_env = pi_half_pulse_env + base_unit_env.repeat(REPEAT_NUM) + idle_env.repeat(4)

# # e) 基于最终的包络创建 APE 微波序列
# raw_mw_sequence = MicrowaveSequence(envelope=full_sequence_env, carrier_freq=carrier_freq * 2 * np.pi, phi=-np.pi*0.3)
# drag_corrector = DRAGCorrector(anh=ANH_GHZ * 2 * np.pi, alpha=0.5)
# mw_sequence = drag_corrector.apply(raw_mw_sequence)
...
# 先构造基本单元的sequence
pi_half_pulse_seq = MicrowaveSequence(envelope=pi_half_pulse_env, carrier_freq=OMEGA_Q_GHZ * 2 * np.pi, phi=-np.pi / 2)
inverse_pi_half_pulse_seq = MicrowaveSequence(envelope=pi_half_pulse_env, carrier_freq=OMEGA_Q_GHZ * 2 * np.pi, phi=-np.pi / 2 + np.pi)
idle_seq = MicrowaveSequence(envelope=idle_env, carrier_freq=OMEGA_Q_GHZ * 2 * np.pi, phi=0)

# 标准的APE序列
# base_seq = inverse_pi_half_pulse_seq + idle_seq + pi_half_pulse_seq
# raw_mw_sequence = pi_half_pulse_seq + base_seq.repeat(REPEAT_NUM) + idle_seq.repeat(4)

# 这是一段好玩的序列：相当于走一个大圆
equator_pi_pulse_seq = MicrowaveSequence(envelope=pi_half_pulse_env, carrier_freq=OMEGA_Q_GHZ * 2 * np.pi, phi=-np.pi / 4).repeat(2)
pi_half_pulse_seq_xaxis = MicrowaveSequence(envelope=pi_half_pulse_env, carrier_freq=OMEGA_Q_GHZ * 2 * np.pi, phi=0)
base_seq = pi_half_pulse_seq + idle_seq + equator_pi_pulse_seq  + idle_seq + pi_half_pulse_seq_xaxis
raw_mw_sequence = base_seq.repeat(REPEAT_NUM) + idle_seq.repeat(4)
...
def save_animation():
    ...
    print("正在保存动画到文件...")
    try:
        ani.save("temp/ape_animation.mp4", writer='ffmpeg', fps=24, dpi=100)
        print("动画已保存到 temp/ape_animation.mp4")
    except Exception as e:
        print(f"保存动画时出错: {e}")
        try:
            # 尝试使用pillow保存为gif
            ani.save("temp/ape_animation.gif", writer='pillow', fps=10)
            print("动画已保存为 temp/ape_animation.gif")
